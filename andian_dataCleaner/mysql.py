...
class Mysql:
    ...
    def getCursor(self, sql):
        ...
        # 建立数据库连接
        self.db = pymysql.connect(host=self.host, user=self.user, password=self.pwd, database=self.dbname, port=self.pt)
    ...
    def readtransformerdata(self):
        db = Mysql(host='47.108.51.23', user='root', pwd='data@123456', dbname='electric_monitor')
        sql = "select * from transformer_monitor"
