import argparse
import time
from ahp_gis import gis_benchmark
...
while True:
    ...
    print("#" * 50)
    print("benchmark gis start...")
    weight, flag = read_weight(config, "GIS")
    if flag != 0:
        print("程序异常")
        sys.exit(0)

    # 读取变压器数据
    transformer_data, flag = read_device(config, "GIS")
    if flag != 0:
        print("程序异常")
        sys.exit(0)
    ...
    # 读取目标时间
    target_ti, flag = read_target_time(config, "GIS")
    ...
    ti_all, flag = read_time(config, "GIS")
    if flag != 0:
        print("程序异常")
        sys.exit(0)
    ...
    if is_run:
        toMysqlData_normal, toMysqlData_abnormal = gis_benchmark(
            transformer_data, transformer_criteria, target_ti)
        writeDatabase.write_result(config, toMysqlData_normal,
                                   toMysqlData_abnormal, "GIS")
