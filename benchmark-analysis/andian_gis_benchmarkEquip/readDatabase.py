...
def read_target_time(config, device_type):
    if device_type == "GIS":
        sql = '''select input_time from andian_gis_monitor ORDER BY input_time DESC LIMIT 0, 1'''
...
def read_time(config, device_type):
    if device_type == "GIS":
        sql1 = '''select time from andian_benchmark_normal_gis
        ORDER BY time DESC LIMIT 0, 1'''

        sql2 = '''select time from andian_benchmark_abnormal_gis
              ORDER BY time DESC LIMIT 0, 1'''
...
def read_device(config, device_type):
    ...
    if device_type == "GIS":
        ...
        sql = '''select equip_id, equip_name, pd_peak, discharge_amplitude, discharge_times, gis_pressure, 
        micro_water_content, gis_temperature, risk_level 
        from
        (select * from andian_gis_monitor ORDER BY input_time DESC LIMIT 60) t 
        GROUP BY t.equip_id ,t.equip_name,t.pd_peak,t.discharge_amplitude,t.discharge_times,t.gis_pressure,t.micro_water_content,t.gis_temperature,t.risk_level,t.input_time
...
def read_weight(config, device_type):
    if device_type == "GIS":
        ...
        sql = '''select pd_peak, discharge_amplitude, discharge_times, gis_pressure, 
              micro_water_content, gis_temperature from andian_gis_weight'''
    else:
