import argparse
import time
from ahp_transformer import transformer_benchmark
...
while True:
    print("#" * 50)
    print("benchmark trans start...")

    weight, flag = read_weight(config, "变压器")
    if flag != 0:
        print("程序异常")
        sys.exit(0)

    # 读取变压器数据
    transformer_data, flag = read_device(config, "变压器")
    if flag != 0:
        print("程序异常")
        sys.exit(0)
    transformer_criteria = create_criteria(weight.values)

    # 读取目标时间
    target_ti, flag = read_target_time(config, "变压器")
    ...
    ti_all, flag = read_time(config, "变压器")
    if flag != 0:
        print("程序异常")
        sys.exit(0)
    ...
    if is_run:
        toMysqlData_normal, toMysqlData_abnormal = transformer_benchmark(transformer_data, transformer_criteria, target_ti)
        writeDatabase.write_result(config, toMysqlData_normal, toMysqlData_abnormal, "变压器")
        print("The results have been written to the database")
