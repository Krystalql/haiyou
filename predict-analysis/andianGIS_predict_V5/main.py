...
parser.add_argument('--interval', type=int, required=False, default=1800,
                    help='Run interval')
parser.add_argument('--variablesNumber', type=int, required=False, default=5, help='variablesNumber')  #required表示参数是否可以省略
...
args.variables = ["pd_peak","discharge_amplitude","gis_pressure","micro_water_content", "gis_temperature"] #变量名
args.dbModelsNumber = [18, 6]  # 指定模型数据库的个数及参数个数
args.rank = ['IV', 'III', 'II', 'I']  # 分数等级，可修改个数及形式，如：['I', 'II', 'III']，['差', '一般', '正常', '良好']
args.scoreRange = [90, 100]  # 划分等级得分数范围，如只想让50-100的区间分数显示为[I-V]级，可以设置为[50.0,100.0]
