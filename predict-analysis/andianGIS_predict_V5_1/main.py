...
parser = argparse.ArgumentParser(
    description='LSTM-transformer predict'
)  #对 ArgumentParser 构造方法的调用都会使用 description= 关键字参数
...
parser.add_argument('--interval',
                    ...
                    default=1800,
                    help='Run interval')
parser.add_argument('--variablesNumber',
                    type=int,
                    required=False,
                    default=5,
                    help='variablesNumber')  #required表示参数是否可以省略
parser.add_argument('--predictTimeStep',
                    type=int,
                    required=False,
                    default=6,
                    help='predictTimeStep')  #预测未来总步数
parser.add_argument('--inputTimeStep',
                    type=int,
                    required=False,
                    default=12,
                    help='inputTimeStep')  # 一次输入步数
parser.add_argument('--outputTimeStep',
                    type=int,
                    required=False,
                    default=1,
                    help='outputTimeStep')  # 一次输出步数
parser.add_argument('--single_numLayers',
                    type=int,
                    required=False,
                    default=1,
                    help='single_numLayers')  # 单变量层数，2
parser.add_argument('--single_hidden_layer_size',
                    type=int,
                    required=False,
                    default=64,
                    help='single_hidden_layer_size')  # 单变量隐藏层节点个数，128
parser.add_argument('--multi_numLayers',
                    type=int,
                    required=False,
                    default=1,
                    help='multi_numLayers')  # 多变量层数，2
parser.add_argument('--multi_hidden_layer_size',
                    type=int,
                    required=False,
                    default=64,
                    help='multi_hidden_layer_size')  # 多变量隐藏层节点个数，128
parser.add_argument('--maxNormalValue',
                    type=float,
                    required=False,
                    default=9999.0,
                    help='maxAbnormalValue')  # 最大正常值范围，超过该值系统进行容错
parser.add_argument('--minNormalValue',
                    type=float,
                    required=False,
                    default=0.0,
...
args.variables = [
    "pd_peak", "discharge_amplitude", "gis_pressure", "micro_water_content",
    "gis_temperature"
]  #变量名
args.dbModelsNumber = [18, 6]  # 指定模型数据库的个数及参数个数
args.rank = ['IV', 'III', 'II',
             'I']  # 分数等级，可修改个数及形式，如：['I', 'II', 'III']，['差', '一般', '正常', '良好']
args.scoreRange = [30,
                   110]  # 划分等级得分数范围，如只想让50-100的区间分数显示为[I-V]级，可以设置为[50.0,100.0]
