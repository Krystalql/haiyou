...
class MultiLstm:
    ...
    def predict(self, tid,x):
        ...
        if len(x) == self.timesteps + self.frd:
            ...
            scoreModel = MyLstm(self.input_size, self.scoreOutput_size, self.hidden_layer_size, self.num_layers,
                                self.predict_steps)
            scoreModel.load_state_dict(torch.load(PATH, map_location=torch.device('cpu')))
            scoreModel.eval()
            score = []
            with torch.no_grad():
    ...
    def scoreNormalization(self, data):
        ...
        for i in range(self.input_size):
            if self.normalVariable[self.variables[i]][0] - self.normalVariable[self.variables[i]][1] == 0.0:
                temp = 0.001
            else:
                temp = self.normalVariable[self.variables[i]][0] - self.normalVariable[self.variables[i]][1]
