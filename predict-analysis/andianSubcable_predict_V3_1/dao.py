...
class DAO:
    ...
    def queryWarnData(self, equip_id):
        ...
        if len(dataList1) == 0 or len(dataList2) == 0:
            print(
                equip_id +
                "设备的告警值为空，请到eem_subcable_info和eem_subcable_section_configuration数据库及时完善"
            )
            return {}
        else:
    ...
    def insertWarnResult(self, equip_id, monitor_id, data, equipName):
        ...
        if result:
            ...
            self.mysql1.updateOperation(
                sql,
                tuple(data.tolist()) + (equip_id, monitor_id))
        else:
            ...
            row = self.mysql1.insertOperation(
                sql, (equip_id, monitor_id, equipName) + tuple(data.tolist()))
            if row == 0:
                print("插入失败")
    ...
    # 定义插入模型图的参数
    def insertModelParam(self, equip_id, equip_name, monitor_id, dbNumber,
                         paramNumber, data):
        ...
        affectedRows = self.mysql1.updateOperation(
            sql,
            tuple(data) + (equip_id, monitor_id))
        if affectedRows > 0:  # 海缆数据存在，更新成功。
            return
        else:  # 海缆记录不存在，插入数据
            para = "insert into test_predict_sc_model_" + str(
                dbNumber) + " (equip_id,equip_name,monitor_id,"
            s = " values (%s,%s,%s,"
            for i in range(1, paramNumber + 1):
                para += "parameter" + str(i) + ","
                s += "%s,"
            sql = para[:-1] + ")" + s[:-1] + ")"
            self.mysql1.insertOperation(
                sql, (equip_id, equip_name, monitor_id) + tuple(data))
