    ...
    def run_transformer(self):
        ...
        try:
            ...
            for tid, equipName in transformInfo:  # 对所有海缆进行遍历
                ...
                subCableKeySector = myDao.querySubCableKeySector(
                    tid)  # 获得区段id和关键点id信息
                if len(subCableKeySector) == 0:
                    continue
                for section_id, monitorList in subCableKeySector.items(
                ):  # 对一条海缆所有区段进行遍历
                    for monitor_id in monitorList:  # 对一个区段关键点进行遍历
                        subCableData, ti = myDao.queryVariable(
                            tid, monitor_id, self.inputTimeStep)
                        if len(subCableData) == 0 or len(
                                subCableData) != self.inputTimeStep:
                        ...
                        if monitor_id not in maxData.keys():
                            ...
                            maxData[monitor_id][maxData[monitor_id] ==
                                                0.] = 1.0
                        for i in range(self.variablesNumber):
                            result = mySingleLstm.predict(
                                tid, subCableData[:, i], self.variables[i])
                            # print("result:", result)
                        ...
                        singlePredictResult = np.array(
                            singlePredictResult).transpose(1, 0)
                        singleNormPredictResult = np.array(
                            singleNormPredictResult).transpose(1, 0)
                        if len(warnData
                               ) is 0 or section_id not in warnData.keys():
                            sinWarnPreResult = np.zeros(
                                singlePredictResult.shape, dtype="float")
                            sinWarnPreResult.fill(0.5)
                            warnData[section_id] = np.array(self.warn)
                        else:
                            sinWarnPreResult = warnData[section_id] / maxData[
                                monitor_id]
                            sinWarnPreResult[sinWarnPreResult > 1.0] = 1.0
                            sinWarnPreResult = sinWarnPreResult.reshape(
                                -1, self.variablesNumber)
                            sinWarnPreResult = np.repeat(sinWarnPreResult,
                                                         self.predictTimeStep,
                                                         axis=0)
                        # --------------------多变量预测------------------#
                        MultiData = np.concatenate(
                            [subCableData, singlePredictResult], axis=0)
                        ...
                        singlePredictData = np.concatenate([
                            singlePredictResult, singleNormPredictResult,
                            score, rank, sinWarnPreResult
                        ],
                                                           axis=1)
                        # singlePredictData = np.concatenate(
                        #     [singlePredictResult, score, rank], axis=1)
                        # 告警值插入数据库
                        myDao.insertWarnResult(tid, monitor_id,
                                               warnData[section_id], equipName)
                        # 插入数据库
                        # 先删除前n-1条单变量和多变量的记录
                        # myDao.deleteSingleVariable(tid, monitor_id, (self.predictTimeStep - 1) * 1)
                        # 单变量插入历史表
                        myDao.insertVariable(tid, equipName, monitor_id,
                                             singlePredictData, ti)
                        # --------------------生成模型参数------------------#
                        for i in range(len(self.dbModelsNumber)):
                            myDao.insertModelParam(
                                tid, equipName, monitor_id, i + 1,
                                self.dbModelsNumber[i],
                                generateModelParam(
                                    self.args.dbModelsNumber[i]))
