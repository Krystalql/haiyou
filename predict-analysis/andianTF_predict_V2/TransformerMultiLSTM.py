...
class MultiLstm:
    ...
    def scoreNormalization(self, data):
        ...
        for i in range(self.input_size): #input_size = args.variablesNumber  # 输入维度=变量个数 =7
            if float(self.normalVariable[self.variables[i]][0]) - float(self.normalVariable[self.variables[i]][1]) == 0.0:
                temp = 0.001
                #如果最大值和最小值相同，则temp=最大值-最小值=0，但temp要做分母，所以不能为0，取0.001
            else:
