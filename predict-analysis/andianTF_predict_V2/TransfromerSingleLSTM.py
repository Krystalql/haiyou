...
class SingleLstm:
        ...
        if len(x) == self.timesteps:  # timesteps = inputTimeStep输入步数= 12
            for e in x:  # 对每个数进行检查
                if e > self.maxNormalValue or e < self.minNormalValue:  #maxNormalValue :default=9999.0  minNormalValue: default=0
                    print("单变量" + variable + "数据格式异常，进行容错，异常值：" +
                          str(e))  #str() 函数将对象转化为适于人阅读的形式
                    return error
            # 处理数据，归一化
            ...
            while len(predict_y
                      ) < self.frd:  #predict_y存放了预测值，若预测值个数小于6就继续预测，即为每次预测6步。
                seq = torch.FloatTensor(
                    test_inputs[-self.timesteps:]
                )  #[-self.timesteps:]取最后12位数据作为输入，torch.FloatTensor将list、numpy转化为tensor.
                with torch.no_grad(
                ):  #torch.no_grad() 是一个上下文管理器，被该语句 wrap 起来的部分将不会track 梯度。不会进行反向传播的求导
            ...
            actual_predictions = self.DeNormalization(
                predict_y, variable)  #self.调用类中的函数前要加self.
            # 检查预测的数据和容错
            actual_predictions[
    ...
    def Normalization(self, data, varId):
        if float(self.normalVariable[varId][0]) - float(
                self.normalVariable[varId][1]) == 0.0:
            temp = 0.01
        else:
            temp = float(self.normalVariable[varId][0]) - float(
