...
class DAO:
    # 初始化函数，初始化连接列表
    def __init__(self):
        # self.mysql1 = db.MMYSQL(host='47.108.220.99',
        #                        port = int(23548),
        #                        user='dqjc_swpu',
        #                        pwd='swpu@123456',
        ...
        self.mysql1 = db.MMYSQL(host='127.0.0.1',
                               user='root',
                               pwd='123456',
                               dbname='dqjc_zj_business')
    ...
    def queryTransformerInfo(self): #从数据库中获取变压器信息
        ...
        dataList, row = self.mysql1.queryOperation(sql, ()) #queryOperation数据库查询函数 (0,)这里的0给上面的 equip_type_id = 0，查询变压器
    ...
    def queryVariable(self, tid, step): #query查询 Variable变量
        ...
        dataList, row = self.mysql1.queryOperation(sql, (tid, step)) #queryOperation数据库查询函数 返回一个二维元组和行数
        ...
        result = []
        for x in reversed(dataList): #reversed()函数的作用是返回一个反转(倒序)的（元组、列表、字符串、range）。反转的原因：最新的时间点在最下面，要把它倒过来。
    ...
    def queryTime(self, tid, step): #查询时间
        ...
        if len(dataList) == 0:
            return None
        ti = dataList[0][0] + datetime.timedelta(minutes=-180) ##返回第一个时间点的前180分钟？？??
        return ti
    ...
    def insertVariable(self, tid, equipName, data, ti):
        ...
        for temp in data:
            ...
            ti2 = ti.strftime("%Y-%m-%d %H:%M:%S")  #.strftime转换为特定的时间格式 如：2021 9 25 17：11：30
                                                   #%Y 四位数的年份表示（000-9999）
                                                   #%m 月份（01-12）
                                                   #%d 月内中的一天（0-31）
    ...
    # 定义插入模型图的参数
    def insertModelParam(self, equip_id, equip_name, dbNumber, paramNumber, data):
...
#sql=update test_predict_tf_model_str(dbNumber) set parameter1 =%s,parameter2 =%s,parameter3 =%s,parameter4 =%s,parameter5 =%s,parameter6 =%s where equip_id = %s
        ...
        affectedRows = self.mysql1.updateOperation(sql, tuple(data)+(equip_id,))
...
#sql=insert into test_predict_tf_model_1 (equip_id,equip_name,parameter1,parameter2,parameter3,parameter4,parameter5,parameter6) values (%s,%s,%s,%s,%s,%s,%s,%s)
            self.mysql1.insertOperation(sql, (equip_id, equip_name) + tuple(data))
